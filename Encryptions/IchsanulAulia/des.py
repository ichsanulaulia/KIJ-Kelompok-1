...
class des():
    ...
    def encrypt(self, pt, rkb, counter):
        # Initial Permutation
        ...
        pt = self.permute(pt, initial_perm, 64)
        ...
        for i in range(0, 16): 
            ...
            result = self.xor(left, self.f_function(right, ss[i]))
            left = result
            # Swapper
            if(i != 15):
        ...
        if(counter == 1):
            ...
            return text_from_bits(cipher_text)

    def f_function(self, right, ss):
        # Expansion D-box: Expanding the 32 bits data into 48 bits
        right_expanded = self.permute(right, exp_d, 48)
            
        # XOR RoundKey[i] and right_expanded
        xor_x = self.xor(right_expanded, ss)
    ...
    def generateKey(self):
        # Key generation
        # --hex to binary
        self.keys = hex2bin(self.keys)

        # getting 56 bit key from 64 bit using the parity bits
        key = self.permute(self.keys, keyp, 56)
